/*----------------------------------------------------------------------------
	...
	Author  : Tom Stevelt
	Date    : 06/12/2019
	Synopsis: Report stocks with strong ADX and diff of MinusDI and PlusDI.
...
int main ( int argc, char *argv[] )
{
	...
	StartMySQL ( &MySql, "invest" );
 	...
 	sprintf ( OutFileName, "%s/adx_%d.dat", TEMPDIR, getpid() );
	...
	switch ( ReportOptions.Format )
	{
 	...
 	switch ( ReportOptions.Format )
 	{
 		...
 		case RPT_FORMAT_TEXT:
			...
			if ( ReportMember == 0 )
			{
 				snprintf ( xbuffer, sizeof(xbuffer), "ADX Trend" );
			}
			else
			{
 				snprintf ( xbuffer, sizeof(xbuffer), "ADX Trend %ld %s", ReportMember, xmember.xmname );
			}
			switch ( BuySellAll )
			{
				case 'B':
					strcat ( xbuffer, " - Buy Signal" );
					break;
				case 'S':
					strcat ( xbuffer, " - Sell Signal" );
					break;
				case 'A':
					strcat ( xbuffer, " - Buy and Sell" );
					break;
			}
