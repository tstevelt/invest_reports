/*----------------------------------------------------------------------------
	Program : rpt_breakout.c
	Author  : Tom Stevelt
	Date    : 03/15/2019
	Synopsis: Report stocks which have strong price and volumn increase.
...
int main ( int argc, char *argv[] )
{
	...
	StartMySQL ( &MySql, "invest" );
 	...
 	sprintf ( OutFileName, "%s/breakout_%d.dat", TEMPDIR, getpid() );
	...
	switch ( ReportOptions.Format )
	{
 	...
 	switch ( ReportOptions.Format )
 	{
 		...
 		case RPT_FORMAT_TEXT:
			...
			fprintf ( bfp, "Price Increase %.2f%%\n", PriceCutoff );
			fprintf ( bfp, "Volume Increase %.2f%%\n", VolumeCutoff );
			fprintf ( bfp, "Minimum Avg Vol %.0f\n", MinimumAvgVol );
 			nsFclose ( bfp );
 			snprintf ( xbuffer, sizeof(xbuffer), "Breakout" );
