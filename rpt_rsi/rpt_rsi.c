/*----------------------------------------------------------------------------
	...
	Author  : RSI report
	Date    : 05/27/2019
	Synopsis: Report stocks by RSI, by sector or by industry.  Can find 
				either top or bottom performers.
				Available on the website screeners page.

	Who		Date		Modification
	---------------------------------------------------------------------
	tms		09/24/2023	Changed from Relative Strength to RSI and
						added SelectStyle 
----------------------------------------------------------------------------*/
//     Invest report
// 
...
int main ( int argc, char *argv[] )
{
	...
	switch ( ReportOptions.Format )
	{
		...
		case RPT_FORMAT_TEXT:
			...
			{
				case GROUP_NONE:
					fprintf ( bfp, "Not grouped, " );
					break;
				case GROUP_SECTOR:
					fprintf ( bfp, "Grouped by Sector, " );
					break;
				case GROUP_INDUSTRY:
					fprintf ( bfp, "Grouped by Industry, " );
					break;
			}
			switch ( SelectStyle )
			{
				case SELECT_TOP:
					fprintf ( bfp, "top %d %s.\n", ReportCount, GroupMode == GROUP_NONE ? "overall" : "each" );
					break;
				case SELECT_BOTTOM:
					fprintf ( bfp, "bottom %d %s.\n", ReportCount, GroupMode == GROUP_NONE ? "overall" : "each" );
					break;
				case SELECT_MINIMUM:
			...
			nsFclose ( bfp );
			snprintf ( xbuffer, sizeof(xbuffer), "RSI Screener" );
