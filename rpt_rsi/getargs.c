...
static void Usage ()
{
	printf ( "USAGE: rpt_rsi index [-group {none|sector*|industry}] [options]\n" );
	SindexUsage ( 1 );
	printf ( "OPTIONS\n" );
...
void getargs ( int argc, char *argv[] )
{
	...
	for ( xa = 2; xa < argc; xa++ )
	{
		if ( xa + 1 < argc && nsStrcmp ( argv[xa], "-group" ) == 0 )
		{
			xa++;
			if ( nsStrcmp ( argv[xa], "none" ) == 0 )
			{
				GroupMode = GROUP_NONE;
			}
			else if ( nsStrcmp ( argv[xa], "sector" ) == 0 )
			{
				GroupMode = GROUP_SECTOR;
			}
			else if ( nsStrcmp ( argv[xa], "industry" ) == 0 )
			{
				GroupMode = GROUP_INDUSTRY;
			}
			else
			{
				Usage ();
