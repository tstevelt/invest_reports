/*----------------------------------------------------------------------------
	Program : rpt_cooper.c
	Author  : Tom Stevelt
	Date    : 07/03/2019
	Synopsis: Implement Jeff Cooper 'Hit and Run' screeners
...
int main ( int argc, char *argv[] )
{
	...
	StartMySQL ( &MySql, "invest" );
 	...
 	sprintf ( OutFileName, "%s/cooper_%d.dat", TEMPDIR, getpid() );
	...
	switch ( ReportOptions.Format )
	{
 	...
 	switch ( ReportOptions.Format )
 	{
 		...
 		case RPT_FORMAT_TEXT:
 			...
 			nsFclose ( bfp );
 			snprintf ( xbuffer, sizeof(xbuffer), "Cooper 'Hit and Run'" );
