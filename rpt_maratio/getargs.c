...
static int Usage ()
{
	printf ( "USAGE: rpt_maratio index -avgs # # -days # [options]\n" );
	SindexUsage ( 1 );
	printf ( " -avgs # #  = numbers can be 5, 10, 20, 50, 100 and 200\n" );
	printf ( " -days #    = cross over during past # of days\n" );
	FormatUsage ( 1 );
	printf ( "Options:\n" );
	printf ( " -minimum #.# = minimum percentage to report (default 90)\n" );
	printf ( " -member id\n" );
	printf ( " -d   = Debug\n" );
	exit ( 1 );
}
...
void getargs ( int argc, char *argv[] )
{
	...
	for ( xa = 2; xa < argc; xa++ )
	{
		...
		else if ( xa + 1 < argc && nsStrcmp ( argv[xa], "-minimum" ) == 0 )
		{
			xa++;
			MinimumPercent = nsAtof ( argv[xa] );
		}
	...
	if ( Days1 == 0 || Days2 == 0 || CrossOverSince == 0 )
	{
		Usage ();
	}
}
